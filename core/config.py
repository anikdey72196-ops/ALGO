...
class TradingConfig(BaseModel):
    ...
    temporal_veto_toxic: bool = Field(
        default=False,
        description="Whether to block trade entries during statistically and ML-predicted TOXIC_AVOID time/session windows (False = scale risk down without hard veto).",
    )
    temporal_max_loss_probability: float = Field(
        default=0.65,
        ge=0.50,
        le=0.95,
        description="Threshold of predicted P(Loss) to categorize window as TOXIC_AVOID and trigger defensive veto/scale.",
    )
